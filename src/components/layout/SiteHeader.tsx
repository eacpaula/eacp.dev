  ...
  return (
    <>
      <header className="sticky top-0 z-40 border-b border-border bg-surface-overlay/95 backdrop-blur-xl">
        <div className="mx-auto grid w-full max-w-[80rem] grid-cols-[minmax(0,1fr)_auto] items-center gap-4 px-4 py-4 sm:px-6 md:flex md:flex-wrap md:items-center md:justify-between">
          <Link
            to="/"
            hash="hero"
            className="sketch-badge sketch-badge-muted w-fit text-[0.68rem] transition hover:border-accent hover:text-accent-muted md:order-1"
          ...
          {resumeLink.kind === 'asset' ? (
            <button
              ...
              className="sketch-button-primary inline-flex items-center justify-center justify-self-end rounded-control px-4 py-2 text-sm font-semibold tracking-[0.03em] transition focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent md:order-3"
            >
              {resumeLink.label}
            </button>
          ) : (
          ...
          <nav
            aria-label="Primary"
            className="sketch-shell col-span-2 flex w-full flex-nowrap items-center gap-2 overflow-x-auto rounded-control p-1.5 text-sm md:order-2 md:col-auto md:w-auto"
          >
            {navItems.map((item) => (
